        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('tables.store') }}" class="space-y-6" x-data="{ selectedClient: null, clientSearch: '', showClientResults: false, clients: [] }">
                        @csrf
                        ...
                        <div>
                            ...
                            <select id="table_number" name="table_number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                @foreach($tableNumbers as $tableNum)
                                    <option value="{{ $tableNum }}" {{ old('table_number', $selectedTable) == $tableNum ? 'selected' : '' }}>
                                        {{ $tableNum }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="booking_time" name="booking_time" type="time" class="mt-1 block w-full" :value="old('booking_time', $selectedTime)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('booking_time')" />
                        </div>

                        <div>
                            <x-input-label for="duration" value="Длительность (минуты)" />
                            <x-text-input id="duration" name="duration" type="number" min="30" step="30" class="mt-1 block w-full" :value="old('duration', 60)" required />
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('tables.index', ['date' => $selectedDate]) }}" class="inline-flex items-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-400 focus:bg-gray-400 active:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Отмена
                            </a>
                        </div>
                    </form>
